...
var otrovnaqbulkax=50 , otrovnaqbulkay=50;
var dulina=1;
...
function update() {
	console.log("ima dve qbulki koito sa cherveni.ednata e otrovna a drugata ne e.opitaite se da hvashtate samo poleznata qbulka.");
...
//	}
	if(applex==myX && appley==myY){
		...
		otrovnaqbulkax=Math.floor(Math.random()*80)
		otrovnaqbulkay=Math.floor(Math.random()*60)
	}
	if(otrovnaqbulkax==myX && otrovnaqbulkay==myY){
		dulina=dulina-1;
		otrovnaqbulkax=Math.floor(Math.random()*80)
		otrovnaqbulkay=Math.floor(Math.random()*60)
		applex=Math.floor(Math.random()*80)
...
function draw() {
	...
	context.fillRect(otrovnaqbulkax*10,otrovnaqbulkay*10,9,9);
}
